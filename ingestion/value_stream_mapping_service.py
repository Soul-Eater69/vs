...
class _AzureValueStreamResolver:
    ...
    def resolve_one(self, name: str) -> Optional[str]:
        ...
        try:
            from src.clients.azure_search import IDPAISearchRequest
            ...
            # Keep original value if Azure lookup is unavailable.
        ...
        # Not a valid value stream match
...
def canonicalize_value_stream_names(vs_names: list[str]) -> list[str]:
    """
    Dedupe and normalize value-stream names to canonical Azure entity_name values
    via BM25 lookup on ValueStream nodes.

    Invalid/unmatched names are dropped when lookup is available.
    If Azure lookup is unavailable, falls back to dedupe-only behavior.
    """
...
def _normalize_theme_summary_to_value_stream(summary: str) -> str:
    text = (summary or "").strip()
    if not text:
        return ""

    try:
        from ingestion.metadata import clean_value_stream_name
        text = clean_value_stream_name(text)
    except Exception:
        pass
    ...
    if not vs_links:
        ...
        if themes:
            ...
            for theme in themes:
                ...
                summary = _normalize_theme_summary_to_value_stream(summary_raw)
...
def resolve_value_stream_epics_mapping(ticket_data: dict, classified_links: dict) -> dict[str, Any]:
    ...
    Creates a mapping of:
    {
        ...
        "epic_names": [...],
        "summary": {
            "num_value_streams": int,
            "num_unique_epics": int,
            "num_vs_with_epics": int
        }
    }

    Args:
        ticket_data: Dict with keys 'themes' and 'epics' (from JiraValueStreamClient.get_ticket_data)
        classified_links: Dict with link classifications (for consistency with other resolvers)

    Returns:
        Dict with value stream to epics mapping and summary statistics
    """
    # First get the value stream mapping
    ...
    # Extract epics from ticket data
    epics_raw = list((ticket_data or {}).get("epics") or [])

    # Normalize and dedupe epics
    ...
    for epic in epics_raw:
        ...
        summary = _normalize_theme_summary_to_value_stream(summary_raw) if summary_raw else ""
    ...
    # Build mapping of value stream to epics
    # For now, associate all epics with all value streams (you can refine this logic)
    ...
    return {
        ...
        "summary": {
            ...
            "num_unique_epics": len(set([e.get("id") for e in epics_normalized])),
