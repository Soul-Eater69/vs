...
function passSelectedRows(value: unknown): SelectedVS[] {
  return arrayOfRecords(recordOf(value).selected_value_streams).map((row, index) => ({
    entity_id: String(row.entity_id ?? row.id ?? index),
    entity_name: String(row.entity_name ?? row.name ?? ''),
    confidence: Number(row.confidence ?? 0),
    reason: String(row.reason ?? row.rationale ?? ''),
  })).filter((row) => row.entity_name);
}

function passRejectedRows(value: unknown): RejectedVS[] {
  return arrayOfRecords(recordOf(value).rejected_candidates).map((row, index) => ({
    entity_id: String(row.entity_id ?? row.id ?? index),
    entity_name: String(row.entity_name ?? row.name ?? ''),
    reason: String(row.reason ?? row.drop_reason ?? row.rationale ?? ''),
  })).filter((row) => row.entity_name);
}
...
const TAB_ITEMS: Array<{ key: Tab; label: string }> = [
  ...
  { key: 'llmPasses', label: 'LLM Passes' },
...
const PIPELINE_STAGE_ORDER: PipelineStage[] = ['extract', 'prepare', 'historical', 'merge', 'llm_select', 'finalize'];

const PIPELINE_GROUPS: PipelineNode[][] = [
  [{ key: 'extract', stage: 'extract', title: 'Extract', subtitle: 'Read idea card', icon: 'file' }],
  [
    { key: 'condense', stage: 'prepare', title: 'Condense', subtitle: 'LLM summarize', icon: 'bolt' },
    { key: 'semantic', stage: 'prepare', title: 'VS Search', subtitle: 'Azure VS index', icon: 'search' },
  ],
  [{ key: 'historical', stage: 'historical', title: 'Historical', subtitle: 'FAISS lookup', icon: 'database' }],
  [{ key: 'merge', stage: 'merge', title: 'Merge', subtitle: 'Combine evidence', icon: 'sliders' }],
  [
    { key: 'directLlm', stage: 'llm_select', title: 'Direct LLM', subtitle: 'Idea-card fit', icon: 'spark' },
    { key: 'historicLlm', stage: 'llm_select', title: 'Historic LLM', subtitle: 'Pattern fit', icon: 'database' },
  ],
  [{ key: 'finalize', stage: 'finalize', title: 'Finalize', subtitle: 'Filter + rescue', icon: 'check' }],
];
...
function normalizePipelineStage(step: Step): PipelineStage | null {
  ...
  if (step === 'extract' || step === 'prepare' || step === 'historical' || step === 'merge' || step === 'llm_select' || step === 'finalize') {
    return step;
  }
  return null;
}

function stageStatus(step: Step, stage: PipelineStage, stepOnError: Step) {
  const active = normalizePipelineStage(step === 'error' ? stepOnError : step);
...
function PipelineConnector() {
  return (
    <div className="hidden min-h-[72px] items-center xl:flex">
      <div className="h-[2px] w-8 rounded-full bg-[var(--vse-accent)]/70" />
    </div>
  );
}
...
function PipelineGraph({
  ...
  const activeStage = normalizePipelineStage(step);

  return (
    <div className="flex flex-1 flex-wrap items-center gap-3">
      {PIPELINE_GROUPS.map((group, index) => (
        <div key={group.map((item) => item.key).join('-')} className="flex items-center gap-3">
          <div className={cn('grid gap-2', group.length > 1 ? 'min-w-[210px]' : 'min-w-[196px]')}>
            {group.map((item) => (
              <StageCard
                key={item.key}
                title={item.title}
                subtitle={item.stage === activeStage && stepLabel ? stepLabel : item.subtitle}
                icon={item.icon}
                status={stageStatus(step, item.stage, stepOnError)}
              />
            ))}
          </div>
          {index < PIPELINE_GROUPS.length - 1 && <PipelineConnector />}
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-4 px-4 py-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button className="vse-filter-chip">Min Confidence: 70%+ Ã—</button>
  ...
  return (
    <div className="space-y-4 px-4 py-4">
      ...
      <div className="flex items-center justify-between px-2 text-[0.95rem] text-[var(--vse-muted)]">
        <div>Showing 1â€“5 of 256 results</div>
        <div className="flex items-center gap-2">
          {['â€¹', '1', '2', '3', 'â€¦', '52', 'â€º'].map((item) => (
...
function PassList({
  ...
  const selected = passSelectedRows(output);
  const rejected = passRejectedRows(output);
  const pretty = JSON.stringify(output ?? {}, null, 2);

  return (
    <Panel className="overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--vse-border)] px-5 py-4">
        <div>
          <h3 className="text-[1.25rem] font-semibold text-[var(--vse-ink)]">{title}</h3>
          <p className="mt-1 text-[0.94rem] text-[var(--vse-muted)]">{subtitle}</p>
        </div>
        <div className="flex gap-2">
          <Badge tone="mint">{selected.length} selected</Badge>
          <Badge>{rejected.length} rejected</Badge>
        </div>
      </div>

      <div className="grid gap-0 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)]">
        <div className="border-b border-[var(--vse-border)] p-5 xl:border-b-0 xl:border-r">
          <div className="text-sm font-semibold uppercase tracking-[0.08em] text-[var(--vse-muted)]">Selected</div>
          <div className="mt-4 space-y-3">
            {selected.length ? selected.map((item) => (
              <div key={`${title}-${item.entity_id}-${item.entity_name}`} className="rounded-[16px] border border-[var(--vse-border)] bg-white px-4 py-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="font-semibold text-[var(--vse-ink)]">{item.entity_name}</div>
                  {item.confidence > 0 && <Badge tone="blue">{percent(item.confidence)}%</Badge>}
                </div>
                {item.reason && <p className="mt-2 text-[0.94rem] leading-6 text-[var(--vse-muted)]">{item.reason}</p>}
              </div>
            )) : (
              <div className="rounded-[16px] border border-[var(--vse-border)] bg-[var(--vse-soft)] px-4 py-3 text-[0.96rem] text-[var(--vse-muted)]">
                No selections returned by this pass.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-5">
          <div className="text-sm font-semibold uppercase tracking-[0.08em] text-[var(--vse-muted)]">Raw Pass Output</div>
          <pre className="vse-code-block mt-4 max-h-[460px]">{pretty}</pre>
        </div>
      </div>
    </Panel>
  );
...
function LlmPassesView({
  ...
  return (
    <div className="space-y-4 px-4 py-4">
      <div>
        <h2 className="text-[2rem] font-semibold tracking-[-0.03em] text-[var(--vse-ink)]">LLM Pass Outputs</h2>
        <p className="mt-2 text-[1rem] text-[var(--vse-muted)]">Compare the direct idea-card selector with the historical pattern selector.</p>
      </div>
      <PassList
        title="Direct LLM"
        subtitle="Selections from the direct idea-card and semantic candidate pass."
        output={directOutput}
      />
      <PassList
        title="Historical LLM"
        subtitle="Selections from the historical analog and pattern gap-fill pass."
...
export default function Home() {
  ...
  const rawResponseRecord = recordOf(result?.raw_response);
  const directLlmOutput = result?.direct_llm_output ?? rawResponseRecord.direct_pass ?? null;
  const historicalLlmOutput = result?.historical_llm_output ?? rawResponseRecord.historical_gap_pass ?? null;
        ...
        <main className="px-5 py-5">
          <div className="space-y-4">
            ...
            <Panel className="overflow-hidden">
              ...
              {tab === 'llmPasses' && <LlmPassesView directOutput={directLlmOutput} historicalOutput={historicalLlmOutput} />}
