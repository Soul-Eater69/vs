"""Canonical attachment routing for ingestion."""
...
def _select_additional_excluded(
    scored: list[dict],
...
def build_routing_artifact(
    ...
    light_extract = [
        ...
        if _attachment_id(att) not in {_attachment_id(x) for x in full_extract}
        and _attachment_id(att) not in {_attachment_id(x) for x in excluded}
        and att.get("triage_score", 0) > 10
    ]
...
__all__ = ["build_routing_artifact", "get_routing_candidates", "route_attachments"]
