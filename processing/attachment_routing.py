...
def _cheap_peek(file_bytes: bytes, ext: str) -> dict:
    if ext in ("pptx", "ppt"):
        from processing.extraction.pptx import cheap_peek_pptx
        return cheap_peek_pptx(file_bytes)
    if ext == "pdf":
        from processing.extraction.pdf import cheap_peek_pdf
        return cheap_peek_pdf(file_bytes)
    if ext in ("docx", "doc"):
        from processing.extraction.docx import cheap_peek_docx
