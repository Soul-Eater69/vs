...
def merge_native_and_ocr_text(
    ...
    """Combine native extraction with OCR fallback without duplicating obvious overlap."""
    native_clean = clean_markdown_text(native_text)
    ocr_clean = clean_markdown_text(ocr_text)

    if not native_clean:
        return ocr_clean
    if not ocr_clean:
        return native_clean
    if ocr_clean in native_clean:
        return native_clean
    if native_clean in ocr_clean:
        return ocr_clean

    merged_lines: list[str] = []
    seen: set[str] = set()

    for source in (native_clean, ocr_clean):
