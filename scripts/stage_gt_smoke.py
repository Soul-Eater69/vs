...
def _print_summary(ticket_gt: dict[str, Any], *, focus_theme: str) -> None:
    ...
    print("Child stage resolution:")
    for item in focused.get("child_issue_stage_resolution_debug") or []:
        print("-" * 40)
        print(f"child_key: {item.get('child_key')}")
        print(f"raw_summary: {item.get('raw_summary')}")
        print(f"cleaned_stage_name: {item.get('cleaned_stage_name')}")
        print(f"canonical_stage: {item.get('canonical_stage')}")
        print(f"included: {item.get('included')}")
        print(f"confidence: {item.get('confidence')}")
        print(f"match_method: {item.get('match_method')}")
        print("candidates:")
        pprint(item.get("candidates") or [])
