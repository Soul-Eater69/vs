...
def test_review_pool_system_prompt_is_balanced_operational_prompt() -> None:
    ...
    assert "strong implied operational impacts" in prompt
    assert "OPERATIONAL IMPACT REASONING" in prompt
    assert "These are reasoning categories, not automatic selections" in prompt
    ...
    assert "foundational streams" not in lower_prompt
    assert "target streams" not in lower_prompt
