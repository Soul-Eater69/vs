...
def _extract_attachment_leaves(file_bytes: bytes, attachment: dict, cfg: Any) -> list[dict]:
    ...
    for idx, raw in enumerate(extracted.get("chunks", []) or []):
        ...
        header_hierarchy = _build_header_hierarchy(section_title, slide_num, page_num, page_range, slide_range)
...
def _build_header_hierarchy(
    ...
    locator = _locator_label(slide_num=slide_num, page_num=page_num, page_range=page_range, slide_range=slide_range)
    if locator:
