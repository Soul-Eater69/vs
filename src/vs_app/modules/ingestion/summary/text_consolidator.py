...
from vs_app.modules.ingestion.attachments.router import (
...
async def consolidate_ticket_text(
    ...
    if resolved_source.text:
        ...
        source_name = (
            "description link" if resolved_source.origin == "description_link" else "attachment"
        )
...
def _is_summary_idea_card_candidate(att: dict, text: str) -> bool:
    ...
    name = " ".join(
        str(value or "") for value in (att.get("display_text"), att.get("filename"))
    ).strip()
